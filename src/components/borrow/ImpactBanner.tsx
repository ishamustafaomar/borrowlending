  ...
  return (
    ...
    >
      ...
      <p className="relative mt-2 font-display text-xl sm:text-2xl font-bold leading-snug text-primary">
        You've shared{" "}
        <span className="text-coral">{Math.round(items)}</span> items —{" "}
        <span className="text-coral">${Math.round(saved).toLocaleString()}</span>{" "}
        saved and <span className="text-coral">{Math.round(not)}</span> fewer
        things bought.
      </p>
